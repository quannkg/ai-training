...
# Xử lý
# X, y = preprocessing.build_training_data(df, max_number=MAX_NUMBER)
X, y = preprocessing.build_training_data(df, max_number=45)

# Mô hình
# ml_model = model.build_model(input_dim=X.shape[1], output_dim=MAX_NUMBER)
ml_model = model.build_model(input_dim=X.shape[1], output_dim=45)
ml_model.fit(X, y, epochs=1368, batch_size=16, validation_split=0.2)

# Dự đoán
suggested_numbers = predict.predict_next(ml_model, df)

# In kết quả
# formatted = f'{PREFIX} K1'
